...
std::string GetEnvironmentVar(const char *variableName)
{
    ...
    std::vector<wchar_t> value(MAX_PATH);
                                                ...
                                                std::string *errorOut)
{
    ...
    switch (searchType)
    {
        ...
        {
            libraryModule = LoadLibraryW(Widen(libraryName).c_str());
            if (libraryModule)
            {
				if (errorOut)
				{
        ...
        case SearchType::SystemDir:
        {
            ...
            std::string libraryNameStr = libraryName;
            std::replace(libraryNameStr.begin(), libraryNameStr.end(), '/', '\\');
            libraryModule =
                LoadLibraryExW(Widen(libraryNameStr).c_str(), nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
            break;
        }
